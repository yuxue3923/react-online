...
class EchartsTest extends Component {
    ...
    initTree() {
        ...
        //通过父组件传过来的值
        var transData=this.props.eventsOption
        ...
        if(transData){
            const { setEchartData } = this.props;
            setEchartData({
                type: 'echartsecdataSuccess',
                payload: {
                    courselist:myChart.getOption().series[0].data[0].children,
                }
            })
        }
    }
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        setEchartData:(state) => dispatch(state),
    };
}
